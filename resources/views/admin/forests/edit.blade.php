<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('admin.forests.update', $forest) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="provinsi" id="provinsi_input"
                                        value="{{ old('provinsi', $forest->provinsi) }}"
                                ...
                                <div>
                                    ...
                                    <input type="text" name="pulau" id="pulau_input"
                                        value="{{ old('pulau', $forest->pulau) }}"
                                        class="mt-1 block w-full rounded-md border-gray-300 bg-gray-100 shadow-sm focus:border-forest-500 focus:ring-forest-500 cursor-not-allowed"
                                        readonly>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="text" name="latitude" id="lat_input"
                                            value="{{ old('latitude', $forest->latitude) }}"
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="longitude" id="lng_input"
                                            value="{{ old('longitude', $forest->longitude) }}"
                                            class="mt-1 block w-full rounded-md border-gray-300 bg-gray-100 shadow-sm font-mono text-xs cursor-not-allowed"
                                            readonly required>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="number" step="0.01" name="luas_hektar"
                                        value="{{ old('luas_hektar', $forest->luas_hektar) }}"
                                ...
                                <div>
                                    ...
                                    <select name="status_konservasi"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-forest-500 focus:ring-forest-500">
                                        <option value="konservasi" {{ $forest->status_konservasi == 'konservasi' ? 'selected' : '' }}>Hutan Konservasi</option>
                                        <option value="lindung" {{ $forest->status_konservasi == 'lindung' ? 'selected' : '' }}>Hutan Lindung</option>
                                        <option value="produksi" {{ $forest->status_konservasi == 'produksi' ? 'selected' : '' }}>Hutan Produksi</option>
                                        <option value="konversi" {{ $forest->status_konservasi == 'konversi' ? 'selected' : '' }}>Hutan Konversi</option>
                                    </select>
                                </div>

                                <input type="hidden" name="geojson" id="geojson_input" value="{{ $forest->geojson }}">
                            </div>
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mt-2"><strong>Klik pada peta</strong> untuk memperbarui titik lokasi. Provinsi dan Pulau akan terisi otomatis.</p>
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            var map = L.map('map-draw').setView([-2.5489, 118.0149], 5);
            ...
            // Initialize Marker if exists from Lat/Long
            var initialLat = {{ $forest->latitude ?? 'null' }};
            var initialLng = {{ $forest->longitude ?? 'null' }};

            if (initialLat && initialLng) {
                marker = L.marker([initialLat, initialLng]).addTo(map);
                map.setView([initialLat, initialLng], 8);
            }
